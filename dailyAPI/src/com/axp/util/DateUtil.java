...
public class DateUtil
{
	...
	/**
	 * 两个日期的天数差
	 * sdf.parse("2018-11-01 10:14:04"), sdf.parse("2018-11-02 10:04:03")) 没有到24小时 就不算一天 
	 * @param fromDate 起始日期
	 * @param toDate 结束日期
 ...
 */
	 ...
	 * @return
	 */
	public static boolean compTime(String time,String from ,String to){
		try {
			if (time.indexOf(":")<0||from.indexOf(":")<0 || to.indexOf(":")<0) {
				System.out.println("格式不正确");
			}else{
				String[]array1 = time.split(":");
				int total1 = Integer.valueOf(array1[0])*3600+Integer.valueOf(array1[1])*60+Integer.valueOf(array1[2]);
				String[]array2 = from.split(":");
				int total2 = Integer.valueOf(array2[0])*3600+Integer.valueOf(array2[1])*60+Integer.valueOf(array2[2]);
				String[]array3 = to.split(":");
				int total3 = Integer.valueOf(array3[0])*3600+Integer.valueOf(array3[1])*60+Integer.valueOf(array3[2]);
				if((total1-total2>0) && (total3-total1>0)){
					return true;
				}else{
	...
	/**
	 * @param time 格式 时分秒 HH:mm:ss 
	 * @param a  增加时间 目前是增加小时
	 
	 * @param b 分钟
	 * @param c 描述
	 * @return
	 */
	public static Long getDiffTime(String time,int a,int b,int c){
		...
		cal2.set(Calendar.HOUR_OF_DAY, Integer.valueOf(array[0])+a);
		cal2.set(Calendar.MINUTE, Integer.valueOf(array[1])+b);
		cal2.set(Calendar.SECOND, Integer.valueOf(array[2])+c);
		
		return cal2.getTimeInMillis()-cal.getTimeInMillis();
	 ...
	 * @return
	 */
	public static long getDiffHHmmss(String time, String st) {
		
		
		int a = 0;
		try {
			if (time.indexOf(":")<0||st.indexOf(":")<0) {
				System.out.println("格式不正确");
			}else{
				String[]array1 = time.split(":");
				int total1 = Integer.valueOf(array1[0])*3600+Integer.valueOf(array1[1])*60+Integer.valueOf(array1[2]);
	...
	/**
	 * 返回天数差
	 *  sdf.parse("2018-11-01 10:14:04"), sdf.parse("2018-11-02 10:04:03")) 没有到24小时 也算一天的
	 * @param end
	 * @param start
	 * @return
	 */
